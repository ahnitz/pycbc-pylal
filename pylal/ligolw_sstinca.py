...
def inspiral_max_dt(events, e_thinca_parameter):
	...
	return sum(sorted(max(lal.SnglInspiralTimeError(event, e_thinca_parameter) for event in events if event.ifo == instrument) for instrument in set(event.ifo for event in events))[-2:]) + 2. * lal.REARTH_SI / lal.C_SI
